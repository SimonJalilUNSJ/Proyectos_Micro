...
int main(void){
	...
	while(1)
	{
		...
		//Systick no es requerido entonces se deshabilita antes de ir a dormir
		HAL_SuspendTick();

		//Se duerme aqui
		__WFE();

		//Continua desde aca cuando se despierta
		//Habilita el systick
		HAL_ResumeTick();
...
void HHAL_GPIO_EXTI_Callback(uint16_t GPIO_Pin){
